...
public abstract partial class SharedHandsSystem
{
    ...
    /// </summary>
    public bool TryDrop(EntityUid uid, Hand hand, EntityCoordinates? targetDropLocation = null, bool checkActionBlocker = true, bool doDropInteraction = true, HandsComponent? handsComp = null, double dropAngle = 0)
    {
        if (!Resolve(uid, ref handsComp))
            return false;
        ...
        TransformSystem.SetWorldPositionRotation(entity, GetFinalDropCoordinates(uid, origin, target), itemRot + dropAngle);
        return true;
    }
    ...
    /// </summary>
    public Vector2 GetFinalDropCoordinates(EntityUid user, MapCoordinates origin, MapCoordinates target)
    {
    ...
    public virtual void DoDrop(EntityUid uid, Hand hand, bool doDropInteraction = true, HandsComponent? handsComp = null)
    {
        ...
        _renderOrder.SetRenderOrder(entity, EntityManager.CurrentTick.Value);

        if (doDropInteraction)
            _interactionSystem.DroppedInteraction(uid, entity);

        RaiseLocalEvent(entity, new ItemDroppedEvent(uid));
